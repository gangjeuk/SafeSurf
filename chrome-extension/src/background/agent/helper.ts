    ...
    try {
      ...
      if (response?.completion_message?.content?.text) {
        ...
        const transformedResponse = {
          ...
          usage: {
            prompt_tokens: response.metrics?.find((m: unknown) => m.metric === 'num_prompt_tokens')?.value || 0,
            completion_tokens: response.metrics?.find((m: unknown) => m.metric === 'num_completion_tokens')?.value || 0,
            total_tokens: response.metrics?.find((m: unknown) => m.metric === 'num_total_tokens')?.value || 0,
          },
        };
